...
async def gateway():
    ...
    await gateway.update_all_light_status()
    await gateway.update_group_list()
    return gateway

@pytest.mark.asyncio
async def test_connection(gateway):
    # tests if a connection is possible and some lights are returned
    print("test_connection")
    assert gateway.lights()

@pytest.mark.asyncio
async def test_groups(gateway):
...
@pytest.mark.asyncio
async def test_group_info(gateway):
    # tests if the function gateway.group_info() returns the lights in
    # GROUP1_LIGHTS
    group = gateway.groups()[GROUP1]
    lights = ([gateway.lights()[addr] for addr in await gateway.group_info(group)])
...
@pytest.mark.asyncio
async def test_turn_group_on_and_off(gateway):
    ...
    await group.set_onoff(0)
    asyncio.sleep(GROUP_SLEEP)
    await gateway.update_all_light_status()
    for light in lights:
        assert not light.on()
    await group.set_onoff(1)
    asyncio.sleep(GROUP_SLEEP)
    await gateway.update_all_light_status()
    for light in lights:
        assert light.on()
...
@pytest.mark.asyncio
async def test_motion_sensor(gateway):
    motion_sensor_name = "MotionSensor1"
    motion_sensor = get_addr_of_light(motion_sensor_name, gateway)
    for i in range(1000):
        asyncio.sleep(0.1)
        result = await gateway.update_light_status(motion_sensor)
        print(result)

@pytest.mark.asyncio
async def test_light_by_name(gateway):
    # test if the function light_byname returns the correct light
    light = gateway.light_byname(LIGHT1)
    assert light.name() == LIGHT1

@pytest.mark.asyncio
async def test_colors_single_light(gateway):
    # switches the light to red, green and blue and checks if the status
    # is updated accordingly
    light = gateway.light_byname(LIGHT1)
    old_on, old_lum, temp, old_r, old_g, old_b = await gateway.update_light_status(
        light)

    await light.set_onoff(1)
    await light.set_luminance(255, 0)
    await light.set_rgb(255, 0, 0, 0)
    asyncio.sleep(1)
    on, lum, temp, r, g, b = await gateway.update_light_status(light)
    assert colors_almost_equal((r, g, b), (255, 0, 0))

    await light.set_rgb(0, 255, 0, 0)
    asyncio.sleep(1)
    on, lum, temp, r, g, b = await gateway.update_light_status(light)
    assert colors_almost_equal((r, g, b), (0, 255, 0))

    await light.set_rgb(0, 0, 255, 0)
    asyncio.sleep(1)
    on, lum, temp, r, g, b = await gateway.update_light_status(light)
    assert colors_almost_equal((r, g, b), (0, 0, 255))

    await light.set_rgb(old_r, old_g, old_b, 0)
    await light.set_luminance(old_lum, 0)
    await light.set_onoff(old_on)

@pytest.mark.asyncio
async def test_colors_group(gateway):
    ...
    for light, old_rgb in zip(lights, old_rgbs):
        r, g, b = old_rgb
        await light.set_rgb(r, g, b, 0)


# @pytest.mark.skip(reason="takes very long (about 30s)")
@pytest.mark.asyncio
async def test_color_cyle(gateway):
    ...
    old_on, old_lum, temp, old_r, old_g, old_b = await gateway.update_light_status(
        light)
    await light.set_onoff(1)
    await light.set_luminance(100, 0)
    for i in range(512):
        col_val = i % 256
        await light.set_rgb(255 - col_val, 255 - col_val, col_val, 0)
        # asyncio.sleep(0.05)
    await light.set_rgb(old_r, old_g, old_b, 0)
    await light.set_luminance(old_lum, 0)
    await light.set_onoff(old_on)
...
@pytest.mark.asyncio
async def test_temperature_cyle(gateway):
    # changes the color temperature of a light from 2200 kelvin to 6500
    # in steps of 100.
    light = gateway.light_byname(LIGHT1)
    old_on, old_lum, old_temp, old_r, old_g, old_b = await gateway.update_light_status(
        light)
    await light.set_onoff(1)
    asyncio.sleep(0.2)
    for i in range(2200, 6501, 100):
        await light.set_temperature(i, 0)
        asyncio.sleep(0.1)
    await light.set_temperature(old_temp, 0)
    await light.set_onoff(old_on)

@pytest.mark.asyncio
async def test_luminance(gateway):
